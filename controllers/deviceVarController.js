...
const updateDeviceVar = async (req, res) => {
    const {varid, vartype, varaccess, schedid, varformula,varname} = req.body;
    pool.query(
        'UPDATE devicevar SET vartype = $2, varaccess=$3, schedid=$4, varformula=$5, varname=$6, lastaccess=NOW() WHERE varid = $1',
        [varid, vartype, varaccess, schedid, varformula,varname],
        (error, results) => {
            if (error) { 
                handleQueryError(error, req, res);  
            } else {
